...
import com.louisgeek.myscanlogin.qrcode.CameraManager;
import com.louisgeek.myscanlogin.qrcode.PlanarYUVLuminanceSource;
import com.louisgeek.myscanlogin.qrcode.decoding.InactivityTimer;
import com.louisgeek.myscanlogin.tool.QrBarTool;
...
//========================================打开本地图片识别二维码 end=================================
    ...
    private void initDialogResult(Result result) {
        ...
        MyApplication.getInstance().getTcpClient().send("scan_success");

        new AlertDialog.Builder(mContext).setTitle("提示")
                .setMessage("扫描完成：" + result)
                .setPositiveButton("确认登录", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        new Thread(new Runnable() {
                            @Override
                            public void run() {
                                MyApplication.getInstance().getTcpClient().send("login_loading");
                                //
                                ...
                                //模拟 http 请求认证 返回 通过认证
                                if (true) {
                                    MyApplication.getInstance().getTcpClient().send("login_success");
                                    UIToast.show(mContext,"登录成功！");
                                }
                ...
                .setNegativeButton("重新扫描", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                        MyApplication.getInstance().getTcpClient().send("relogin");
                        if (handler != null) {
                            // 连续扫描，不发送此消息扫描一次结束后就不能再次扫描
                            handler.sendEmptyMessageDelayed(R.id.restart_preview, 3_000);
                        }
                    }
                }).create().show();
